...
Private Declare Function GetTickCount Lib "kernel32" () _
    As Long 'API for getting how long windows has been running
...
Private Sub cmdPaste_Click()
    ...
    With frmMain
        Select Case .tbsCodeTabs.SelectedItem.Key
            Case "Code"
                .rtfCode.SelText = txtCopyChars.Text
            Case "Notes"
                .rtfNotes.SelText = txtCopyChars.Text
        End Select
    End With
End Sub
...
Private Sub Wait(Length As Long, Optional Sleep As Boolean = False)
    Dim StartTime As Double 'Start time of the sub
    
    'Get the current time
    StartTime = GetTickCount
    
    'Loop while time taken is less than the length
    Do While GetTickCount - StartTime < Length
...
Private Sub FlashLocated(ByVal LabelIndex As Integer)
    ...
    For Counter = 1 To NoOfFlashes - 1
        ...
        Call Wait(ShortHighLightLength)
